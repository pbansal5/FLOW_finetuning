...
def setup_logger(log_dir, args):
    ...
    log_file = log_dir / f'logs_ours-round2_{args.model}_{args.dataset}_lr{args.lr}.txt'
...
def test(net, testloader, criterion, device, epoch, best_acc, checkpoint_dir, log_print, args):
    ...
    if acc > best_acc:
        ...
        torch.save(state, checkpoint_dir / f'{args.dataset}/ckpt_best2.pth')
...
def main():
    ...
    parser.add_argument('--lp-dir', default='./checkpoints/ours',
                        type=str, help='checkpoint directory')
    ...
    lp_path = lp_dir / f'{args.model}/{args.dataset}/ckpt_best.pth'
    ...
    # 5) Identify the final layer and unfreeze its parameters
    if hasattr(net, 'fc'):
        final_layer = net.fc  # For models with a 'fc' layer
    elif hasattr(net, 'head'):
        final_layer = net.head  # For models with a 'head' attribute
    else:
        raise AttributeError(f"Model {args.model} does not have a recognizable final layer.")
    ...
    for epoch in range(epochs):
        ...
        train_acc = train(net, trainloader, optimizer, criterion, device, epoch + 1, log_print)
    ...
    checkpoint_path = checkpoint_dir / f'{args.dataset}/ckpt_best2.pth'
    if not checkpoint_path.exists():
        ...
        print(f"ImageNet accuracy after {args.dataset} linear-probe:")
