...
EPOCH_CONFIGS = {
    'cifar10': 20,  # 20 epochs for CIFAR-10
    'cifar100': 25,
    'stanford_dogs': 30,
    'flowers102': 25,
    'stanford_cars': 30,
...
def setup_logger(log_dir, args):
    ...
    log_file = log_dir / f'logs_full_{args.model}_{args.dataset}_bestrun.txt'
...
def test(net, testloader, criterion, device, epoch, best_acc, checkpoint_dir, log_print, args):
    ...
    if acc > best_acc:
        ...
        torch.save(state, checkpoint_dir / f'{args.dataset}/ckpt_full_{args.model}_{args.dataset}_lr{args.lr}.pth')
...
def main():
    ...
    parser.add_argument('--seed', default=42, type=int,
                        help='random seed')
    parser.add_argument('--checkpoint-dir', default='./checkpoints/standard_ft',
                        type=str, help='checkpoint directory')
    parser.add_argument('--log-dir', default='./logs/standard_ft',
    ...
    checkpoint_path = checkpoint_dir / f'{args.dataset}/ckpt_full_{args.model}_{args.dataset}_lr{args.lr}.pth'
    if not checkpoint_path.exists():
        ...
        print(f"ImageNet accuracy after {args.dataset} full finetuning:")
